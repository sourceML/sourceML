...
<form name="morceau_form" action="<?= $this->url("users/morceaux/add") ?>" method="post">
...
<?php if($this->out["morceau"]) : foreach($this->out["morceau"]["documents"] as $id_document => $document) : ?>
        ...
        <div class="document" id="document_<?= $id_document ?>">
          ...
          <input type="text" size="40" name="document_url_<?= $id_document ?>" id="document_url_<?= $id_document ?>" value="<?= $document["url"] ?>" />
        </div>
...
<?php endforeach; endif; ?>
    ...
    <li>
      <textarea class="tinymce" cols="40" rows="10" name="description" id="description"><?= $this->out["morceau"]["description"] ?></textarea>
    </li>
